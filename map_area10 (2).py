...
# Function to create a map with specified amenities
def create_map(amenities=None):
    # Initialize the map with Thunderforest Transport Light tiles
    m = folium.Map(
    ...
    for _, row in stations_gdf.iterrows():
        station_name = row.get('station_name', 'Unnamed Station')  # Get station name from 'station_name' column
        folium.CircleMarker(
...
if destination != 'Select your destination':
    ...
    map_object = create_map(amenities=amenities_data)
else:
