...
# Use Streamlit session state to maintain map settings between interactions
if 'center' not in st.session_state:
    st.session_state['center'] = [38.89511, -77.03637]  # Default center on downtown DC
if 'zoom' not in st.session_state:
    st.session_state['zoom'] = 10  # Default zoom level

# Update the center and zoom level based on user interaction
...
def create_map(station_coords, amenities=None):
    # Initialize the map with the current session state for center and zoom
    m = folium.Map(
    ...
    # Capture the updated center and zoom level when the user interacts with the map
    m.on('moveend', lambda: update_map_state(m.get_center(), m.get_zoom()))
